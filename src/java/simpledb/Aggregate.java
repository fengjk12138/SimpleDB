...
public class Aggregate extends Operator {
    ...
    public String aggregateFieldName() {
        // some code goes here
        return childOp.getTupleDesc().getFieldName(aField);
    }
    ...
    public void open() throws NoSuchElementException, DbException,
        ...
        super.open();
        childOp.open();
        Type tmp = childOp.getTupleDesc().getFieldType(aField);
        Aggregator myAgg;
        if (tmp == Type.INT_TYPE) {
            myAgg = new IntegerAggregator(
                    gField,
                    gField == -1 ? null : childOp.getTupleDesc().getFieldType(gField),
                    aField,
                    operator
            );
        } else {
        ...
        while (childOp.hasNext()) {
            myAgg.mergeTupleIntoGroup(childOp.next());
        }
